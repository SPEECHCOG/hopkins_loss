...
The configuration file for train_autoencoder_and_model_hopkins.py.
...
# The name of the autoencoder model that we want to use from the file hopkins_model.py
ae_model = 'MLP_network'
...
result_dir = f'hopkins_ae_results_{data_type}_{experiment_number}'
...
name_of_log_textfile = f'hopkins_ae_{data_type}_trainlog_{experiment_number}.txt'

# The name of the saved model. Please note that this file (and its directory) will be saved under the
# directory result_dir.
best_model_name = f'hopkins_{data_type}_best_encoder_experiment_{experiment_number}.pt'
...
# file (and its directory) will be saved under the directory result_dir.
name_of_result_file = f'hopkins_ae_{data_type}_results_{experiment_number}.npy'
...
# The weight for the reconstruction loss (Hopkins loss weight will be 1 - reconstruction_loss_weight)
reconstruction_loss_weight = 0.75
...
hopkins_loss_input = 'layer_3_output'

# Define our loss functions that we want to use from torch.nn
reconstruction_loss_name = 'MSELoss'
